    ...
    def register_capability_async(
        ...
        if view is not None:
            if capability_path.startswith("textDocumentSync."):
                debug("a textDocumentSync was registered. checking if we need to notify didOpen")
                self._check_did_open(view)

    def unregister_capability_async(self, registration_id: str, registration_path: str) -> None:
        discarded = self.capabilities.unregister(registration_id, registration_path)
