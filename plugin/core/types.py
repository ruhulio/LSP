...
class DocumentSelector:
    ...
    def matches(self, view: sublime.View) -> bool:
        ...
        return any(filter(lambda f: f.matches(view), self.filters)) if self.filters else True
...
class Capabilities(DottedDict):
    ...
    def unregister(
        ...
        if not isinstance(stored_registration_id, str):
            return debug("stored registration ID at", registration_path, "is not a string")
        elif stored_registration_id != registration_id:
            msg = "stored registration ID ({}) is not the same as the provided registration ID ({})"
            return debug(msg.format(stored_registration_id, registration_id))
        else:
